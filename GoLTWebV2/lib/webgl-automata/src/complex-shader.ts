...
interface StateValueNumberConfiguration {
    name: string,
    type: 'number',
    length: number
}

interface StateValueFlagConfiguration {
    name: string,
    type: 'flag'
}

type StateValueConfiguration = StateValueNumberConfiguration | StateValueFlagConfiguration;

interface NeighbourCountConfiguration {
    matrix: number[][]; // MxN matrix where M and N are odd numbers
    valuefn: string; // expression code
    type: 'int' | 'float';
    overflow: 'wrap' | 'border' | 'zero';
}

interface ShaderConfiguration {
    state: StateValueConfiguration[];
    neighbourCounts: Record<string, NeighbourCountConfiguration>;
    program: string; // code to calculate the cell state for the next generation
}

interface StateVariableNumberLayoutSpan {
    byte: number;
    from: number;
    to: number;
}

interface StateVariableFlagLayout {
    type: 'flag',
    byte: number,
    bit: number
}

interface StateVariableNumberLayout {
    type: 'number',
    spans: StateVariableNumberLayoutSpan[]
}

type StateVariableLayout = StateVariableFlagLayout | StateVariableNumberLayout;

type StateLayout = Record<string, StateVariableLayout>;


function makeStateLayout(config: StateValueConfiguration[]): StateLayout {
    let byte = 0;
    let bit = 0;
    ...
    for (let v of config) {
        if (v.type === 'flag') {
            ...
            const spans: StateVariableNumberLayoutSpan[] = [];
            while (remaining > 0) {
...
interface NeighbourSettingElement {
    name: string;
    valuefn: Expression;
    type: 'int' | 'float';
    overflow: 'wrap' | 'border' | 'zero';
...
interface NeighbourSetting {
    data: Float32Array;
    width: number;
    height: number;
    elements: NeighbourSettingElement[];
}


function makeNeighbourSetting(config: Record<string, NeighbourCountConfiguration>, scope: Scope): NeighbourSetting {
    ...
    const elements: NeighbourSettingElement[] = [];
    let index = 0;
    for (const key in config) {
...
function trimCode(code: string, indent_level: number = 0) {
    const lines = code
        .split('\n')
...
function generateStateGetterFlag(name: string, layout: StateVariableFlagLayout): string {
    return trimCode(`
...
function generateStateSetterFlag(name: string, layout: StateVariableFlagLayout): string {
    return trimCode(`
...
function generateStateGetterNumber(name: string, layout: StateVariableNumberLayout): string {
...
function generateStateSetterNumber(name: string, layout: StateVariableNumberLayout): string {
    if (layout.spans.length === 1) {
        const span = layout.spans[0];
        const length = span.to - span.from + 1;
        return trimCode(`
...
function generateNeighbourCountFunc(setting: NeighbourSettingElement) {
    let code = '';
...
function generateProgram(program: Program, indentLevel: number): string {
    let code = '';
...
function generateShader(config: ShaderConfiguration) {
    const scope: Scope = {
        variables: {}
    };
...
// example
const config: ShaderConfiguration = {
    state: [
        {
            name: 'foo',
            type: 'number',
